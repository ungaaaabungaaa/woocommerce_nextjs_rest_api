...
// pull in all the data through the api
// build a simple filter function for json parshing and get the whole object
// display the filtered item

// jsx Part
// load in the banner
// load in the bread crumsb
  ...
  useEffect(() => {
    ...
    switch (sortOption) {
      ...
      case "low-high":
        result.sort(
          (a, b) =>
            parseFloat(a.price || a.regular_price) -
            parseFloat(b.price || b.regular_price)
        );
        break;
      case "high-low":
        result.sort(
      ...
      result = result.filter((product) =>
        product.attributes?.some(
          (attr) =>
            ...
            attr.options.some(
              (option) => option.toLowerCase() === selectedColor.toLowerCase()
            )
        )
      );
    }
      ...
      result = result.filter((product) =>
        product.attributes?.some(
          (attr) =>
            ...
            attr.options.some(
              (option) => option.toLowerCase() === selectedSize.toLowerCase()
            )
        )
      );
    }
